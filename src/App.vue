<template>
  <div class="clock" :style="colorSetting"><Clock /></div>
  <Weather :style="colorSetting" :loc="loc" :unit="unit" />
  <transition name="fade">
    <div v-if="settingsOpen" class="settings-menu">
      <Settings
        :style="colorSetting"
        @update-storage="updateStorage"
        @change-color-settings="saveColorSettings"
        @change-bg-color-settings="saveBgColorSettings"
        @update-background="saveBgImage"
        @check-bg-option="checkBgOption"
      />
    </div>
  </transition>
...
export default {
  ...
  methods: {
    ...
    saveColorSettings() {
      ...
      this.colorSetting.color = localStorage.getItem('color')
      if (localStorage.getItem('color') === null) {
        this.color = '#FC5608'
        this.colorSetting.color = '#FC5608'
      }
    },
    saveBgColorSettings() {
    ...
    saveBgImage() {
      this.bgImgSrc = localStorage.getItem('bg')
      const body = document.querySelector('body')
